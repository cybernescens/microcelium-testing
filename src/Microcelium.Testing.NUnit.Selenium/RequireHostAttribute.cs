...
public abstract class RequireHostAttribute : TestActionAttribute 
{
  ...
  public override void BeforeTest(ITest test)
  {
    ...
    if (test.Fixture is IRequireLogValidation)
      builder.ConfigureLogging(DefaultLogValidationConfiguration);
    ...
    if (test.Fixture is IRequireLogValidation validator)
    {
      logContext = serviceScope.ServiceProvider.GetRequiredService<LogValidationContext>();
      validator.LogContext = logContext;
  ...
  protected virtual void DefaultLogValidationConfiguration(HostBuilderContext ctx, ILoggingBuilder logging)
  {
    logging.Services.AddScoped<LogValidationContext>();
    logging.Services.AddScoped<LogValidationContextLoggerProvider>();
    logging.Services.AddScoped<LogMessageBuffer>();
  }
