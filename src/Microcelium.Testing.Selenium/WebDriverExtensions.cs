...
public static class WebDriverExtensions
{
  ...
  /// <returns></returns>
  public static TSite UsingSite<TSite>(this IWebDriver webDriver, WebDriverConfig config, ILogger? log = null)
  ...
  public static void ImportCookies(this IWebDriver webDriver, CookieContainer cookieContainer, Uri site) =>
    webDriver.ImportCookies(cookieContainer.GetCookies(site));
  ...
  /// <returns></returns>
  public static bool DefinitivelyWaitForAnyAjax(this IWebDriver driver, ILogger? log = null, TimeSpan? seconds = null)
  {
    var result = driver.WaitForJavascriptResult("(window.jQuery || { active: 0 } ).active", 0, log, seconds) &&
      driver.WaitForJavascriptResult(
        "((window.jQuery.ajax || { ajax: null } ).ajax || { active: 0 }).active",
        0,
        log,
        seconds) &&
      driver.WaitForJavascriptResult(
        "$ && $('.dataTables_processing').is(':visible') === false ? 0 : 1",
        0,
        log,
        seconds) &&
  ...
  /// <returns></returns>
  public static FileInfo WaitForFileDownload(
    this IWebDriver driver,
    string directory,
    string fileMask,
    ILogger? log = null,
    TimeSpan? timeout = null) =>
    new DownloadHelper(driver, log).WaitForFileDownload(new DirectoryInfo(directory), fileMask, timeout ?? TimeSpan.FromSeconds(60));
  ...
  /// <param name="timeout"></param>
  /// <returns></returns>
  public static bool WaitForJavascriptResult<TResult>(
    this IWebDriver driver,
    string javascript,
  ...
  /// <param name="timeout"></param>
  /// <returns></returns>
  public static bool WaitForDifferentJavascriptResult<TResult>(
    this IWebDriver driver,
    string javascript,
    TResult expectedResult,
    ILogger? log = null,
  ...
  /// <param name="timeout"></param>
  /// <returns></returns>
  public static IAlert GetAlert(this IWebDriver driver, ILogger? log = null, TimeSpan? timeout = null)
  {
